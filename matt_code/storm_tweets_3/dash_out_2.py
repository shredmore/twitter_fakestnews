...
print ''
print 'Total number of occurrences of #fakenews in past 20 minutes: %s' % (count1)
print ''
...
cur.execute("SELECT word, count from tweetwordcount3 where word not like '#%' and word not like '@%' order by count desc limit 20")
records = cur.fetchall()
for rec in records:
    print rec[0], rec[1]
print ""
...
cur.execute("SELECT word, count from tweetwordcount3 WHERE word like '#%' and word not like '#fakenews' order by count desc limit 20")
records = cur.fetchall()
for rec in records:
    print rec[0], rec[1]
print ""
...
cur.execute("SELECT word, count from tweetwordcount3 WHERE word LIKE '@%' order by count desc limit 20")
records = cur.fetchall()
for rec in records:
    print rec[0], rec[1]
print ""
