...
print 'Top 30 trending words in the past 20 minutes from Tweets with #fakenews:'
cur.execute("SELECT word, count from tweetwordcount2 where length(word) > 4 and word not like '_fakenews' and word not like '_FakeNews' and word not like '_FAKENEWS' and word not like '_Fakenews' order by count desc limit 30")
records = cur.fetchall()
for rec in records:
    print (rec[0], rec[1]), ", "
