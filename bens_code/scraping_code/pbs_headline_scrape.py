...
page = requests.get("http://www.pbs.org/newshour/")
...
for i in soup.find_all(class_="top-story-title"):
	bel = i.find_all('a')
	for t in bel:
		headlines.append(t.text.strip())

#get last used headline id and add 1
my_file = Path("./pbs.csv")
if my_file.is_file():
	last_line = subprocess.check_output(["tail", "-1", "pbs.csv"])
...
with open('pbs.csv', 'a', newline='') as csvfile:
